package com.bridgerputnam.dayThree;
...
public class DayThree {
    ...
    private Point getIntersectingPoint(Point p1, Point p2, Point p3, Point p4) {
    ...
    private Point[] inputToWire(String input) {
        String[] instructions = input.split(",");
        Point[] wire = new Point[instructions.length + 1];
        wire[0] = this.port;
        for(int i = 0; i < instructions.length; i++) {
            String instruction = instructions[i];
            char direction = instruction.charAt(0);
            int moves = Integer.parseInt(instruction.substring(1));
            Point newPoint = movePoint(wire[i], direction, moves);
            wire[i + 1] = newPoint;
        }
        return wire;
    }

    private Point movePoint(Point start, char direction, int moves) {
        int x = start.x;
        int y = start.y;
        switch(direction) {
            case 'U':
